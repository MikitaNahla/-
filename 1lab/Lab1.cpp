...
char checkSign(string value1, string value2) {
    ...
    int temp_value1 = binToInt(value1);
    int temp_value2 = binToInt(value2);
    if (fabs(temp_value1) >= fabs(temp_value2)) {
        result = value1[0];
    }
    return result;
}
...
string sum(string value1, string value2) {
    ...
    if (rem == '1') {
        ...
        result = compare(result, temp, rem = '0');
    }
    if (sign == '1') {
        result = '1' + result;
...
string comp(string value1, string value2) {
    ...
    for (int i = value2.size() - 1; i >= 0; i--) {
        ...
        result = sum("0" + result, "0" + temp_result);
        temp_result = "";
    }
    result = sign + result;
    return result;
...
void findMantissaNumber(string& mantissa, string exponent, double temp) {
    int M = pow(2, SIZE_OF_MANTISSA) * temp;
...
string doubleToBin(double number) {
    ...
    double temp = formuleInMantissa(fabs(number), lower_limit, upper_limit);
    findExponentNumber(exponent, lower_limit);
    findMantissaNumber(mantissa, exponent, temp);
