...
function binToNum(value){
    ...
    for (let i = 0; i < value.length; i++) {
        num += 2 * value[i] ** (value.length - i - 1)
    }
    return num
}
...
function findRowsQuan(quantityOfVariables){
    return 2**(quantityOfVariables + 1)
}

//Нахождение столбцов таблицы
function findColumnQuan(quantityOfVariables, quantityOfOperations){
    return quantityOfVariables + quantityOfOperations
}
